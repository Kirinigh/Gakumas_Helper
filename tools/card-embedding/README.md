...
`diagnose_arena_candidate_recall.py` 只读已安装代码、已有事务和冻结参考，比较原图库与内存中的完整界面参考候选；不训练、不保存候选图库、不调用游戏。保存的源帧不等于原始三帧身份快照，输出不能直接作为训练标签或上线依据。

输入必须提供 `--runtime-root`、可重复的 `--dataset-manifest` 和新的 `--output` 路径。三类证据入口互斥：

- `--evidence-root`：已有事务目录，另须提供私有 `--legacy-review` JSON。以事务目录名为键，只有 `association_confirmed: true` 才进入正例；旧标题证据另须显式 `legacy_full_title_confirmed: true`。未复核及错槽关联默认排除，不在代码中保存私有事务编号。
- `--fresh-manifest`：核对新场景的源图与详情图、散列、版本、标题、原点击和卡框，并按成员场景冻结分组。
- `--manual-manifest`：接收人工确认槽位的截图，保留客户区、卡框、详情和未知采集信息，不伪造原点击或采集版本。

`--arena-only` 保持基础图库不变，只评估竞技场候选。重复裁图不算独立场景，恒色对照不算真实未知集；候选改善也不代表整轮读取准确率或速度已通过实机验证。输入、复核清单和报告仅保存在私有目录，不随公开源码分发。
...
`evaluate_coarse_sampling.py` 只在内存中比较 `top50`、`top58`、`top62`、中央缺口及下角区保护缺口；不构建或晋级生产图库，不调用模型、Maa 或游戏输入。输入为当前安装根、原固定PNG目录、按批次的扩展manifest、529既有live manifest及私有case JSON。原3388张PNG的有序摘要必须与固定基线来源一致，全部现用参考重新提取的16×8特征必须逐字节复现生产数组，之后才能评估新区域。保留当前阈值及源标签过滤。

必需参数为 `--runtime-root`、`--reference-root`、可重复的 `--extension-manifest`、`--live-manifest`、`--cases`、`--output`。case需提供 `case/image_path/image_sha256/box/truth/plan/slot_index`，图片与报告均保留在私有目录。输出前冻结`.split.json`，拒绝覆盖既有报告。默认执行全部9类plan/槽位参考回归与交错计时；`--geometry-only`单独执行既有evaluator的41项裁框扰动，超出源图边界的输入明确省略，不填充或伪造缺失像素。

时间只覆盖卡槽归一、特征、距离和候选归并，不是完整读取墙钟。参考自回放、已知开发图及同图扰动均不等于独立实机准确率；候选退化到歧义或新增错误唯一时不得据平均改善晋级。任何候选未通过可靠性保持门时都不能进入实机验收。

### 保留旧128点的加权补充取样

同一工具加 `--weighted`，使用相同来源参数，输出到新的私有报告路径；该模式一次执行原样本、全库、几何扰动与交错计时，不能同时传 `--geometry-only`。case另须显式提供 `sample_unit` 和布尔 `ordinary_counterexample`，不得根据识别结果推断普通卡资格。输入清单保存在私有目录，同一原图的不同裁法共用sample_unit；一个普通样本只有全部已登记视野均正确唯一、且至少一个视野原来非正确唯一，才计一次改善。

- A保留16×16缩略图的前9行：旧128格加第8行16格，覆盖56.25%，不是旧58.3%重采样。B再加第9行第0～4列，共149格，是不规则区域。行列从0开始。
- 两区域各比较0.05/0.10/0.15/0.20四个补充权重，仅八个待晋级候选。每个区域先求平均绝对误差，再按`(1-λ)*old + λ*extra`合成；当前帧与图库只计算一次像素差。
- 页面16×16特征完整保留，帧间比较不借用图库差值矩阵。λ=0逐查询核对原误差数组、完整身份结果（包括并列顺序、参考条目和分差）、整卡页面特征与旧参考选择；旧身份不成为新身份接受门。合成三帧规则由定向测试覆盖，当前case集合不具有原始三帧事务，不能宣称真实事务已回放。
- 旧top62控制保持160点均匀平均，等价于旧128点权重80%、补充32点权重20%。与A/B的λ=0.20比较位置，与同区域其他权重比较权重；λ=0及top62均不进入候选排名。
- 全部硬门通过后，按剩余错误唯一数、普通样本正确唯一改善数（降序）、热态P95、补充点数、权重排序。几何扰动与参考自回放另报，不算独立改善。数值合格仍不等于独立验证、页面保护接入或实机节拍合格。
