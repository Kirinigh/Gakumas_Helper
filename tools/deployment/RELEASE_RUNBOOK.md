...
上游同步任务交付的每一笔产品提交，在公开导出时必须同时使用 `--source-update --upstream-sync`。批次来源清单用于确认其属于上游同步，不按作者、标题中的“同步”字样或目录猜测；本地独立改动不得混进同一笔上游提交。导出器保留原中文标题与正文，自动追加标准提交尾注 `Upstream-Sync: true`；公开消息不写内部任务编号。已有手工尾注与该开关同时出现时停止，避免重复或冲突。

派生 git-cliff 配置优先识别这一尾注，统一放入 **🔄 上游同步**，优先于 feat/fix/docs/perf 等类型；本地提交仍按原类型分类。提交尾注只用于生成分类，不显示在公告正文；具体改动、链接与作者保留。新批次接收上游任务时必须逐项核对来源清单、导出开关、公开提交尾注和最终分类，避免遗漏标记。不得因本规则回写既有提交或旧版公告。普通快照导出的历史校验继续接受并保留这些尾注，启动公告不受影响。
...
- 新公开提交的作者及提交者固定为 `Kirinigh <293894750+Kirinigh@users.noreply.github.com>`，使用GitHub账号关联的隐私邮箱与显式日期；历史同时接受原 `Gakumas Helper Release <noreply@gakumas-helper.invalid>` 成对身份，拒绝任意邮箱及作者/提交者混配，不含签名或额外提交头。普通提交保留经核验的 UTF-8 具体标题及可选正文，版本保持父版；发版提交使用发布消息并提高版本。既有历史消息原样验证和保留，不因采用普通提交流程而更名或重写历史；
...
启用 `runtime_refresh` 的库存与旧库存首次衔接采用一次完整包，以交付新更新器。此后增量包总是携带完整 `interface.json`、`python/Lib/site-packages` 和 `python/Scripts` 文件集，包括字节未变的依赖；安装时清除这些目录中 pip 新增但目标未包含的文件，再事务替换为经过校验的目标内容。因此正常期数同步和依赖升级不再造成基底冲突，也不会拼接出新旧依赖混合目录。Python 解释器、原生框架和宿主运行库仍执行原兼容门；所有目标文件仍逐项核验。完整携带依赖会增大增量包，仍沿用 80% 体积阈值；多步链超过阈值时完整下载属于体积选择，不属于依赖冲突。
...
贡献者取公开提交作者并去重，不显示邮箱、不凭作者姓名伪造 `@账号`。新公开提交使用已确认的Kirinigh个人署名；旧公开提交使用统一发行作者时，该名称只代表公开记录，不代表还原了真实个人贡献者；不得自动标为“首次贡献”。需要个人署名时先取得明确、可公开的归属证据。历史归一化作者不因公告生成而重写。
...
用户明确要求应用源码补丁且保持当前版本号时，先从冻结源码和已验证依赖生成完整候选，再使用现有安装器的显式维护选项：

```powershell
& $Installer -CandidatePath $VerifiedCandidate -SameVersionSourceUpdate
```

该选项要求当前清单、界面配置、根版本记录与候选版本一致，候选必须有不同的完整源码提交号。安装目录使用`版本--源码提交号前12位`，沿用原暂存、配置迁移、关键文件核对、Defender扫描、最后切换`current`及回退流程；不覆盖旧版本目录，默认安装行为不变。维护方式和源码身份写入既有`DEPLOYMENT.md`及安装结果，不另建版本台账。

安装后核对候选全部文件、保留的用户配置和本地记录、原版本目录及固定启动入口。这是本机维护，不重新发布同版本tag或替换下载资产；后续正式发版仍使用新版本号和完整发布流程。
