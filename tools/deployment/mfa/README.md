...
- 每次导出固定生成用户所选文件名的一个完整 ZIP。移除原 24.5 MB 自动分包及超限后的重复压缩，不提供分包开关；日志选择、图片范围、包内清单和部分导出状态保持原有规则。
...
回归覆盖两种 Python 日志后端的文件命名、轮转 ZIP、错误元数据、图片开关及时间范围、活动日志共享读取、独占文件、真实目录访问控制、部分导出提示、空目录和超过 42,000 行的日志。固定随机种子生成的三份日志使压缩包超过原 24.5 MB 阈值，分别验证新旧入口均只生成一个完整 ZIP，逐项核对日志、失败图片和清单原字节。目录访问控制用例只在新测试目录内临时拒绝读取并恢复原权限，需要能设置该目录权限的执行端；不接触正式安装或游戏。日志和实际 ZIP 留在测试目录，整个测试不联网、不恢复依赖。
...
GitHub API 资产下载也经过同一等待入口，按请求上的凭据（优先于客户端默认凭据）隔离。成功响应保持 `ResponseHeadersRead` 流式写入；403/429 先检查限流响应头，必要时只读取最多 4096 字符的错误正文。确认限流后抛出专用异常，穿过下载重试层并保留恢复时间提示；普通断线、5xx 和非限流权限错误保留原下载处理。`test_download_transport.py` 同时编译真实共享帮助类，覆盖冷却零请求、首次限流仅一次请求、元数据与资产共享期限、不同凭据隔离及成功资产禁止预缓冲。没有断点续传或持久下载缓存。

## 相邻文件增量更新

`runtime_refresh` 标记的目标库存使用完整刷新范围：`interface.json`、`python/Lib/site-packages/`、`python/Scripts/`。增量必须携带范围内的全部目标文件，即使与前版字节相同；规划与应用前不把这些即将完整替换的文件当作基底冲突，删除目标范围内的额外依赖后再替换。其它程序文件、库存自身、所有下载／解压后的内容与应用后的最终树仍严格核对。没有标记的历史资产保持原规则；兼容标记变化通过一次完整包交付修复，不重写历史资产。此处并非放弃依赖校验，而是避免依赖自更新与程序更新互相冲突。

更新查询结束回到准备阶段；新更新路径按实际读取字节刷新下载百分比与已收／总量，解压、校验和退出应用均有独立提示。下载与校验期间最多显示 99%，不提前宣告整个更新完成。单次流读取超过 60 秒无响应会进入原有共享恢复预算并提示重试，不无限停在旧进度。`test_file_update.py` 覆盖期数和依赖漂移后的单步／链式更新及实际下载回调。

新更新器在同一个 MFA 资源更新入口识别 Release manifest 的 `file_update.protocol=1`，复用本次已取得的版本列表与附件，不逐包重查 Release。首次安装新更新器必须使用完整包；旧完整包没有 `GAKUMAS_HELPER_UPDATE.json` 时不生成相邻增量。现有 Stable/Beta/Alpha 及最高版本选择不变，首版仅对正式版本、安装和数据位于同一目录的布局启用文件增量；其它路径继续原完整更新。

- 仅来源和目标的主/次版本均相同才考虑增量。沿目标 manifest 的前一正式版本向前找链，最多32步，每步校验版本方向、兼容信息及相邻基底清单摘要。增量总压缩体积须小于完整包的80%，否则完整更新；缺链、显式全量、运行库/框架/布局变化也完整更新。
- `GAKUMAS_HELPER_UPDATE.json` 记录正式包全部程序文件的摘要，单独绑定清单自身摘要；运行前逐文件核验，检查清单外程序文件，拒绝仅凭版本号认定本地基底。此库存用于发现真实本地补丁和验证最终包一致性，不保存用户配置或运行数据。
- 相邻 ZIP 内容以 `.gkhdelta` 为扩展名，内含 `changes.json` 的 added/modified/deleted 及新增/修改文件；不生成二进制差分。文件增删改由既有 `UpdateFileTransaction` 承接，新目录纳入回滚，整条链只提交一次；文件锁或删除失败先完整回滚，再在剩余预算内尝试一次完整更新。回滚不完整时保留备份并停止，不继续覆盖。
- 全部增量包下载、摘要和内容集合校验完成后才停止程序。当前客户端先复制独立更新进程所需的宿主和运行库，借同用户命名管道传递凭据与代理（不写清单、日志或命令行）；独立进程等旧进程退出再按序应用，最终全部文件校验通过后只启动目标一次。这也支持替换更新器自身。普通更新失败显示原因；恢复时间提示继续保留。
- 整次操作共享最多2次额外恢复机会，不逐包重置；限流立即停止请求，沿用匿名共享文件和认证隔离。完整包回退不重查版本，网络证书验证不降级。用户配置、日志、运行缓存及 `.local` 整体跳过，不遍历其内部内容。
- 更新中的 pending、最终 success/failed 写入既有运行数据目录的 `update-result.json`；只有最终目标核验并提交事务后写 success。操作暂存位于 `.local/updates/<随机操作ID>`，没有跨次下载缓存或续传；失败备份保留供恢复。此事务覆盖可捕获的文件操作异常，不宣称断电后自动恢复。

验证入口：`test_file_update.py` 编译补丁中的真实规划、下载和事务代码，在隔离目录验证链、回退、文件占用/删除、用户数据保留和共享预算；`test_file_update_worker.py` 使用真实宿主及重建 Core、无界面见证程序验证父进程退出门、自身文件替换和单次重启。测试版本仅为夹具，不对应公开发布或固定安装。

本次交付为已通过离线回归的源码补丁，不能恢复已消耗的 GitHub 服务端额度。现有安装不会自动获得修复；纳入发行客户端仍需按本文件的 Core 构建、双次复建与制品验证流程生成新 bundle。
## 离线连接与识别通知回归

```powershell
python tools/deployment/mfa/test_connection_feedback.py --source-zip "<固定源码 ZIP>" --dotnet "<现有 SDK>\dotnet.exe" --work-dir "<尚不存在的测试目录>"
```

补丁在识别结果生成前不查询详情；空模拟器设备、失效窗口给出明确提示，并保留现有配置允许的刷新和启动恢复。Win32 重试成功会正确继续，取消请求仍能停止连接。

回归校验固定源码 ZIP、实际应用当前补丁，从修改后的 MaaProcessor 提取连接、重试与通知判断方法直接编译。覆盖通知阶段、空设备、无效/零窗口、旧连接状态、自动刷新关闭、重试成功/失败、正常连接及取消。仅替代外部界面和设备操作，不联网、不启动客户端或访问游戏。可使用已有 .NET 8 或更新编译器运行此方法级测试；正式 Core 仍按前文固定构建要求重新构建，方法测试不等于完整客户端已验证或已部署。
