...
为排查竞技场偶发识别失败，包含诊断补丁的版本会在 **2026 年 11 月 1 日前**，限量保存任务已经读取的游戏画面和识别记录：每个安装目录最多 20 组、共 128 MiB，到期或达到限额即停止采样。不会额外点击、截图或自动上传。需要协助定位时，请使用客户端的日志导出功能，检查其中的游戏信息后，再通过与维护者约定的非公开方式提供；不要直接放到公开 Issues。
