...
[![Latest](https://img.shields.io/github/v/release/Kirinigh/Gakumas_Helper?display_name=tag&label=Latest&color=ec4899)](https://github.com/Kirinigh/Gakumas_Helper/releases/latest) [![Windows x86_64](https://img.shields.io/badge/Windows-x86__64-0078D4)](#compatibility) [![AGPL-3.0](https://img.shields.io/badge/License-AGPL--3.0-blue)](LICENSE)

**[下载最新版](https://github.com/Kirinigh/Gakumas_Helper/releases/latest)** · **[使用手册](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/README.md)** · **[更新记录](https://github.com/Kirinigh/Gakumas_Helper/releases)** · **[反馈问题](https://github.com/Kirinigh/Gakumas_Helper/issues/new/choose)**
...
日常、培育与实用工具沿用上游功能；本项目主要增加竞技场的编成读取和胜率选敌。详细区别见[与上游 MaaGakumasu 的区别](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/UPSTREAM_DIFFERENCES.md)。
...
> [!IMPORTANT]
> 竞技场还在持续改进，暂不保证能连续完成每日全部挑战。建议使用 **720×1280 或更大**的窗口，540×960 下的完整日常流程尚未测通。模拟胜率仅供参考，已知问题见[竞技场指南](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/ARENA_WIN_RATE.md)和[更新记录](https://github.com/Kirinigh/Gakumas_Helper/releases)。
...
打开 **[Latest Release](https://github.com/Kirinigh/Gakumas_Helper/releases/latest)**，下载：
...
第一次连接或启动失败时，先看[首次使用](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/GETTING_STARTED.md)和[故障排查](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/TROUBLESHOOTING.md)。
...
Maa 返回的识别图像可能经过缩放，与游戏设置中的窗口尺寸不同；[首次使用](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/GETTING_STARTED.md)有具体说明。
...
| 文档 | 内容 |
| --- | --- |
| [首次使用](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/GETTING_STARTED.md) | 下载、连接、首次竞技场运行与参数设置 |
| [竞技场指南](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/ARENA_WIN_RATE.md) | 己方缓存、选敌、结果记录 |
| [故障排查](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/TROUBLESHOOTING.md) | 常见提示与处理方法 |
| [更新与回滚](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/UPDATE_AND_ROLLBACK.md) | 自动更新、手动迁移、恢复旧版本 |
| [与上游的区别](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/UPSTREAM_DIFFERENCES.md) | 本项目新增和保留的功能 |
| [隐私与安全](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/PRIVACY_AND_SAFETY.md) | 本地记录、截图与反馈前脱敏 |
...
本项目通过自己的 GitHub Releases 更新，暂不支持 Mirror 酱。自动更新仍在测试中。如果更新失败，可以改用完整包安装，操作方法见[更新与回滚](https://github.com/Kirinigh/Gakumas_Helper/blob/main/guides/UPDATE_AND_ROLLBACK.md)。
...
遇到问题，请先搜索 [Issues](https://github.com/Kirinigh/Gakumas_Helper/issues)，再通过[问题反馈表单](https://github.com/Kirinigh/Gakumas_Helper/issues/new/choose)提交。写清以下信息会更容易复现：

- 使用版本、Windows 版本、游戏窗口大小。
- 运行的任务、完整报错，以及出错前做了什么。
- 识别错误发生在哪个舞台、哪名成员、哪张卡或哪个道具槽。

可以附上少量脱敏日志。**请勿上传整份配置、日志包、缓存或原始游戏截图。** 具体说明见[反馈指南](https://github.com/Kirinigh/Gakumas_Helper/blob/main/SUPPORT.md)。

为排查竞技场偶发识别失败，包含诊断补丁的版本会在 **2026 年 11 月 1 日前**，保存任务已经读取的游戏画面和识别记录。每个安装目录最多保留 20 组、共 128 MiB；同一位置的新样本替换旧样本，分类或总额度用满后删除对应范围内最老的样本，继续保留最新记录。已完成定位的问题停止采样，到期后所有探针停止采样。

失败证据与探针样本保留7天，Agent启动时清理过期记录；样本也可能因滚动替换提前删除，需要反馈时请及时导出。不会额外点击、截图或自动上传。请使用客户端的日志导出功能，检查其中的游戏信息后，再通过与维护者约定的非公开方式提供；不要直接放到公开 Issues。

在上游 MaaGakumasu 原版也能复现的问题，可以反馈到[上游 Issues](https://github.com/SuperWaterGod/MaaGakumasu/issues)。暂时不能确定问题归属时，发到本项目即可。

欢迎修正文档、补充测试和提交代码。较大的改动请先开 Issue 讨论；小修复可以直接提交拉取请求 (Pull Request, PR)。开发要求见[贡献说明](https://github.com/Kirinigh/Gakumas_Helper/blob/main/CONTRIBUTING.md)。
