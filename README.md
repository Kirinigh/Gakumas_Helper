...
1. 从 Releases 下载 `MaaGakumasu-win-x86_64-v1.4.9+gkh.260823.zip`，并核对 SHA-256。
...
`v1.4.9+gkh.260823` 使用语义化版本 (Semantic Versioning, SemVer) 构建元数据，与同一基础版本的上游包具有相同优先级。因此首次从对应上游版或其他同基础版本的派生包切入时必须手动安装；后续只有更高 SemVer 优先级的派生版本才能由现有更新入口自动升级。
