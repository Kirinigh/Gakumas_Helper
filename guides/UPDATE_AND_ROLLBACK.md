...
| 更新 | 负责内容 | 使用方式 |
| --- | --- | --- |
| MFA 内置 GitHub 资源更新 | Gakumas Helper 客户端与配套资源 | 正常版本升级的唯一客户端入口；自动选择完整包或适用的文件增量 |
...
| 独立识别资产热更新 | 单独更新模型、图库或规则数据 | 当前不提供；这些资产随客户端版本更新 |
| MirrorChyan | 上游 Maa 的第三方分发入口 | Gakumas Helper 当前不声明派生版 RID，也不通过它分发完整包 |

GitHub 提示请求额度受限时，日志会显示可以再次检查的时间。客户端更新与 RIS 的匿名请求共享这段等待时间，关闭再打开也不会清空；无需反复重试。到期后重启客户端即可再次检查。等待期间仍可使用已有可用资源；请核对竞技场期数，软件不会把旧期数当成已知的新期数。

额度按网络出口计算，同一出口的其他应用也可能占用。程序会减少查询量并按服务器要求暂停请求，但不能保证共享出口永不限流。权限拒绝与响应传输中断会分别处理；后者使用既有有限重试机会。
...
MFA 会按版本、通道、平台和架构选择 GitHub Release 资产。首次获得支持文件增量的新更新器需要完整包；之后，同一主次版本内、所需更新包齐全且本地程序文件未改变时，会在能够明显减少下载量的情况下使用文件增量。其他情况使用完整包，用户不必手动选择。

新更新流程会核对下载摘要（digest）与文件清单，下载和校验完成后才关闭程序，由独立更新进程替换文件，最后重启一次。整次更新共享有限重试机会，遇到请求限流立即停止；增量应用失败时先尝试恢复原文件，再按剩余机会改用完整包。如果恢复未完成，会保留备份并停止。设置、日志、缓存和本地记录继续保留；这不代表支持断电后的自动恢复。旧客户端与不支持增量的渠道或目录布局仍使用原完整包更新流程。

摘要使用 SHA-256。旧完整包流程遇到未提供 digest 的资产会警告后继续；新的清单更新流程要求摘要完整，不能将旧流程的这一行为理解为新流程也会接受缺失摘要的文件。
...
当前版本以非预发布版本设为 GitHub Latest；已使用独立 GKH 版本的用户可保持 MFA 默认 Stable，通过原生入口更新。旧组合版本用户仍需上述一次手工迁移。独立 GKH SemVer 不会清除仓库里的旧组合版本；不要改用 Beta 或 Alpha，否则旧组合版本可能因数值更高而被误判为“更新”。独立更新进程已有离线验证，真实在线下载、应用和重启的完整升级验收仍未完成。
